...
(async () => {
  ...
  if (!message) {
    ...
    await jExec(
      `cd out && git init && git remote add static-html https://github.com/CSIT21/CSIT21-Next-Static.git && git add . && git branch -M static && git commit -m "Deploy from local" && git config http.postBuffer 524288000 && git push static-html static --force`
    );
    console.log("\nCommitting code...");
    await jExec(
